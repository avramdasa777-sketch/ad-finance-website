...
const services = [
  {
    ...
    bullets: ['ביטוח חיים', 'אובדן כושר עבודה', 'ביטוח בריאות'],
  },
  {
...
const uniqueFeatures = [
  ...
  },
  {
    Icon: ClockIcon,
    title: 'זמינות 24/7',
    desc: 'אנחנו כאן בשבילכם בכל שעה — לכל שאלה, התלבטות או החלטה פיננסית.',
  },
  {
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', justifyContent: 'center' }}>
            {infoLinks.map((item, i) => (
              <Link
                key={item.href}
                href={item.href}
                className={`info-link-card-dark info-link-item reveal delay-${Math.min((i + 1) * 80, 600)}`}
              >
                <div style={{
